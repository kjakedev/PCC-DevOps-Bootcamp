...
* Host names set on the nodes: `student-node`, `master-node`, `worker-node01`, `worker-node02`
...
worker-node01 = "18.205.245.169"
worker-node02 = "75.101.200.29"
```
...
Repeat the following steps on `master-node`, `worker-node01` and `worker-node02` by SSH-ing from `student-node` to each cluster node in turn, e.g.

```
ubuntu@studentnode:~$ ssh master-node
...
1. Install the container runtime
    ...
    ```
1.  Configure the container runtime to use CGroups. This part is the bit many students miss, and if not done results in a master-node that comes up, then all the pods start crashlooping.
...
1.  Return to `student-node`
    ...
    Repeat the above till you have done `master-node`, `worker-node01` and  `worker-node02`
...
1.  Copy kubeconfig down from `master-node` to `student-node`

    ```
    mkdir ~/.kube
    scp master-node:~/admin.conf ~/.kube/config
    ```
...
1. Paste the join command that was output by `kubeadm init` on `master-node`
...
1. Repeat the steps 2, 3 and 4 on `worker-node02`
...
1.  Test with curl
    ...
    1. Get the _internal_ IP of one of the nodes
        ...
        ```bash
        kubectl get node worker-node01 -o wide
        ```
        ...
        worker-node01   Ready    <none>   13m   v1.27.4   172.31.26.150   <none>        Ubuntu 22.04.2 LTS   5.19.0-1028-aws   containerd://1.6.12
        ```
